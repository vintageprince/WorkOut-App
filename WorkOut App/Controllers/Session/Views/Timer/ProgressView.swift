...
//  WorkOut App
//
//  Created by Виктория Заканавская on 10.07.2023.
//
...
extension TimerView {
    final class ProgressView: UIView {
        func drawProgress(with percent: CGFloat) {
            ...
            let dotPoint = CGPoint(x: cos(-dotAngle) * radius + centre.x,
                                   y: sin(-dotAngle) * radius + centre.y)
            ...
            let barsPath = UIBezierPath(arcCenter: centre,
                                        radius: barsRadius,
                                        startAngle: startAngle,
                                        endAngle: endAngle,
                                        clockwise: true)
            ...
            let startBarRadius = barsRadius - barsLayer.lineWidth * 5
            ...
            (1...9).forEach { _ in
                ...
                let startBarPoint = CGPoint(
                    x: cos(-barAngle) * startBarRadius + centre.x,
                    y: sin(-barAngle) * startBarRadius + centre.y
                )
                
                let endBarPoint = CGPoint(
                    x: cos(-barAngle) * endBarRadius + centre.x,
                    y: sin(-barAngle) * endBarRadius + centre.y
                )
